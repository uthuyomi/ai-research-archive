...
class ChatRequest(BaseModel):
    ...
    # ★追加：input_pipeline 用（後方互換）
    attachments: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="Optional attachments metadata (files / images)"
    )
...
class _Deps(TypedDict):
    ...
    score_intent: Any
    build_generation_context: Any
    
    # ★追加：attachments を EnvelopeBuilder 経由で組み立てるため
    EnvelopeBuilder: Any
    AttachmentRef: Any
...
def _get_deps() -> _Deps:
  ...
  # ---- input_pipeline（追加）
    ...
    from core.input_pipeline.detector import detect_input
    from core.input_pipeline.preprocess import preprocess_input
    from core.input_pipeline.intent_score import score_intent
    from core.input_pipeline.context import build_generation_context
    
    # ---- vision / ocr（追加）
    from core.executors.media.vision_executor import execute_vision
    from core.executors.media.ocr_executor import execute_ocr
    ...
    _DEPS_CACHE = {
       "ConversationState": ConversationState,
        ...
        "EnvelopeBuilder": EnvelopeBuilder,      # ★ここ追加
        "AttachmentRef": AttachmentRef,          # ★ここ追加
        "AttachmentSource": AttachmentSource,    # ★ここ追加
...
def chat(
    ...
    print("[server] files count =", len(files))
    for f in files:
        print("[server] file:", f.filename, f.content_type)
    deps = _get_deps()
    ...
    # ===== input_pipeline =====
    InputEnvelope = deps["InputEnvelope"]
    detect_input = deps["detect_input"]
    preprocess_input = deps["preprocess_input"]
    score_intent = deps["score_intent"]
    build_generation_context = deps["build_generation_context"]
    
    # ===== vision / ocr =====
    execute_vision = deps["execute_vision"]
    execute_ocr = deps["execute_ocr"]
    ...
    # =========================
    # UploadFile を後段 input_pipeline に渡すため保持
    request.state.upload_files = files
    # 重要：
    # - messages.conversation_id は UUID
    # - user_id は Supabase Auth の user_id(UUID)
    # - session_id は API 上のセッション文字列（補助）
    ...
    session_key="default",
        ...
        # =========================
    # 0.5 UploadFile → AttachmentRef
    # =========================
    AttachmentRef = deps["AttachmentRef"]
    AttachmentSource = deps["AttachmentSource"]
    ...
    detected = detect_input(envelope)
    ...
    import inspect
    print("DEBUG preprocess_input signature:", inspect.signature(preprocess_input))
    ...
    # =========================
    # vision / ocr executors
    # （input_pipeline の延長、人格OSの前）
    # =========================

    vision_result = execute_vision(
    envelope=envelope,
    detection=detected,
    intent=intent_score,
    )
    ...
    generation_context = build_generation_context(
        ...
        ocr=ocr_result,   # ← ★ ここで初めて意味を持つ
    )
    
    try:
        if processed.documents_meta:
            meta = getattr(generation_context, "meta", None)
            if meta is None:
                setattr(generation_context, "meta", {})
                meta = generation_context.meta

            meta["documents_meta"] = processed.documents_meta
    except Exception:
        pass
                
    print(
        "DEBUG generation_context.ocr.executed =",
        getattr(generation_context.ocr, "executed", None)
    )
    
    print("[server] vision:", vision_result.analyzed, "count=", vision_result.image_count, "reasons=", vision_result.reasons)

    print("[server] ocr:", ocr_result.executed, "texts=", len(ocr_result.texts), "reasons=", ocr_result.reasons)
    if ocr_result.executed and ocr_result.texts:
        print("[server] ocr first char_count=", ocr_result.texts[0].get("char_count"))
    
    # =========================
    # 1. セッション取得 / 初期化（補助情報）
    # =========================

    state = SESSION_STATES.setdefault(session_id, ConversationState())
    memory = SESSION_MEMORY.setdefault(session_id, SessionMemory())
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Unknown character_id: {character_id}",
        )

    # =========================
    # 2.5 input_pipeline（UI非分離）
    # =========================
    ...
    intent_raw = IntentParser.parse(text)
    intent = resolve_intent(intent_raw, text=text)
    ...
    past_topic = _extract_past_topic(text) if decided_axis == "past" else None
    hypothetical = _extract_hypothetical(text) if decided_axis == "if" else None
    ...
    try:
        ltm = _get_long_term_memory(deps, session_id)
        if ltm is not None:
            # 実装差異を吸収（best-effort）
            if hasattr(ltm, "add"):
                try:
                    ltm.add(user=text, assistant=final_text)
                except Exception:
                    pass
            elif hasattr(ltm, "append"):
                try:
                    ltm.append({"role": "user", "content": (text or "").strip()})
    ...
    try:
        ...
        meta["input_pipeline"] = {
            # --- LLMに渡された最終判断（生成結果側） ---
            "generation": {
                "mode": str(generation_context.primary_mode),
                "intent": str(generation_context.primary_intent),
                "tone": str(generation_context.tone),
                "detail_level": generation_context.detail_level,
                "confidence": generation_context.confidence,
            },
