...
class ConstraintBuilder:
    ...
    def build(self, instruction: TurnInstruction) -> List[str]:
        ...
        if instruction.mode == TurnMode.REFOCUS:
            ...
            constraints.append(
                "Keep the response structurally short and focused."
            )
            constraints.append(
                "Avoid elaboration, examples, or additional context unless strictly necessary."
            )

        elif instruction.mode == TurnMode.NO_QUESTION:
            # フラグとは独立した強制禁止
            constraints.append(
                "Questions are strictly forbidden in this response."
            )
            constraints.append(
                "Provide only direct statements."
            )
